/* bootstrap.c -- Trinity bootstrap menu
 *
...
#define WHITE 0xFFE0E0E0
#define GREEN 0xFF006400
...
int download(const char *src, const char *dst) {
  ...
  ret = sceHttpCreateTemplate("Trinity/1.00 libhttp/1.1", SCE_HTTP_VERSION_1_1, SCE_TRUE);
  if (ret < 0)
    goto ERROR_EXIT;
...
int print_menu(int sel) {
  ...
  for (i = 0; i < N_ITEMS; i++) {
    psvDebugScreenSetTextColor(sel == i ? GREEN : BLACK);
    printf(" [%c] %s\n", sel == i ? '*' : ' ', items[i]);
  }
...
int module_start(SceSize args, void *argp) {
  ...
  psvDebugScreenInit();
  psvDebugScreenSetBackColor(WHITE);
  psvDebugScreenClear();
