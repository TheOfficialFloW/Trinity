...
// ALL 3.65-3.70 SPECIFIC SECTIONS ARE MARKED WITH "// BEGIN 3.65-3.70"
...
static u32_t wlanbt_code = 0;
static u32_t wlanbt_data = 0;
...
void resolve_imports(unsigned sysmem_base) {
	...
	// BEGIN 3.65-3.70 specific offsets here, used to find Modulemgr from just sysmem base
	...
	// END 3.65-3.70 specific offsets
	...
	for (int i = 0; i < modlist_records; ++i) {
		...
		if (strcmp(info.name, "SceWlanBt") == 0) {
			DACR_OFF(wlanbt_code = (u32_t)info.segments[0].vaddr);
			DACR_OFF(wlanbt_data = (u32_t)info.segments[1].vaddr);
		}
...
void fix_netps_heap() {
	...
	int (*wlanbt_unassign)() = (void*)(wlanbt_code + 0x2a19);
	int (*wlanbt_unlock)() = (void*)(wlanbt_code + 0xe71);
	void *wlanbt_mutex = (void*)((u32_t)wlanbt_data + 0x240 + 0x718);

	sce_psnet_bnet_mutex_lock(global_mutex, 0);
	sce_psnet_bnet_mutex_lock(heap_mutex, 0);

	// Fix free size
	*(uint32_t *)(scenet_data + 0x5670) += 0x1000;

	// Fix free list
	curr = *(chunk_header_t **)(scenet_data + 0x5658);
	while (curr) {
		next = (chunk_header_t *)((char *)curr + curr->size + sizeof(chunk_header_t));

		if (next->magic == MAGIC_FREE) {
			curr->size += next->size + sizeof(chunk_header_t);
			break;
		}
	...
	// Fix busy list
	next = NULL;
	curr = *(chunk_header_t **)(scenet_data + 0x564c);
	while (curr) {
		// Find first wrong link
	...
	*(chunk_header_t **)(scenet_data + 0x565c) = next; // busy list head

	// Find 8 bytes chunk
	void *p = NULL;

	curr = *(chunk_header_t **)(scenet_data + 0x565c);
	while (curr) {
		if (curr->size == 8) {
			p = (void *)((char *)curr + sizeof(chunk_header_t));
			break;
		}
	...
	// Fix wlanbt vptr
	*(uint32_t *)(wlanbt_data + 0x240 + 0x580) = wlanbt_data + 0x240;

	// Unassign all slots
	uint32_t slots = wlanbt_data + 0x240 + 0xb0;
	for (int i = 7; i >= 0; i--) {
		wlanbt_unassign(wlanbt_data + 0x240, slots + i * 0x38);
	}

	// Unlock wlanbt reentrant mutex
	wlanbt_unlock(wlanbt_mutex);
}
